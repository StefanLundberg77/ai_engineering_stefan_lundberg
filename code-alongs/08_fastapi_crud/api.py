...
from data_processing import library_data, save_library
from data_models import Book, Prompt
...
# llm func to create book
@app.post("/books/create_book")
async def create_book(query: Prompt):
    result = await book_agent.run(query.prompt)
    
    new_book = result.output
    
    set_id = max(book.id for book in books)
    
    new_book.id = set_id + 1
